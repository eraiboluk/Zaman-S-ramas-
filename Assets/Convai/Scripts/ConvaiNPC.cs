...
namespace Convai.Scripts
{
    ...
    public class ConvaiNPC : MonoBehaviour
    {
        ...
        public int aboutHistory = 0;
        public int goodPerson = 0;
        public int finishThreshold = 100;
        public bool isCompleted = false;
        public string cname;
        public bool isScored = false;
        public bool playTheAud = false;
        ...
        private async void Start()
        {
            ...
            mission = GetComponent<MissionProgress>();
        }


        private void Update()
        {
            if(aboutHistory+goodPerson >= finishThreshold){
                if(characterID == "3628e172-baf2-11ee-8971-42010a40000f")
                    mission.father=true;
                else if(characterID == "2a0d6c70-9e71-11ee-bc99-42010a40000f")
                    mission.farid=true;
                else if(characterID == "c8380586-9e71-11ee-a6c8-42010a40000f")
                    mission.serena=true;
                else if(characterID == "5b14173c-9e72-11ee-b0a1-42010a40000f")
                    mission.king=true;
            }
        ...
        /// </remarks>
        static string RemoveCoordinates(string input)
        {
            // String içindeki "(x, y)" ifadesini bulma
            int startIndex = input.LastIndexOf("(");
            int endIndex = input.LastIndexOf(")");
            
            // Koordinatları içeren kısmı silme
        ...
        static int[] ExtractCoordinates(string input)
        {
            // Stringin sonundaki "(x, y)" ifadesini almak için indexOf ve substring kullanma
            int startIndex = input.LastIndexOf("(") + 1;
            int endIndex = input.LastIndexOf(")");
            string coordinatesStr = input.Substring(startIndex, endIndex - startIndex);

            // Alınan ifadeyi virgül ile ayırıp iki parça haline getirme
            string[] parts = coordinatesStr.Split(',');

            // Parçaları int'e çevirip dizi olarak geri döndürme
            int[] coordinates = new int[2];
            coordinates[0] = int.Parse(parts[0]);
                ...
                if (GetResponseResponses.Count > 0)
                {
                    ...
                    if (getResponseResponse?.AudioResponse != null)
                    {
                        ...
                        if (getResponseResponse.AudioResponse.AudioData.ToByteArray().Length > 46)
                        {
                            ...
                            /*
                            string[] parts = textDataString.Split(' ');
                            string lastWord = parts[parts.Length - 1];
                            string SecondWord = parts[parts.Length - 2];
                            */
                            try{
                                int[] coo = ExtractCoordinates(textDataString);
                                goodPerson += coo[0];
                                aboutHistory += coo[1];
                                textDataString = RemoveCoordinates(textDataString);
                                getResponseResponse.AudioResponse.TextData = textDataString;
                                Debug.Log("BURADAN GECTI");
                                isScored = true;
                            }catch{
                                Debug.Log("Cok sikintili hata");
                                //isScored = false;
                            }
                            //string a = characterID;
                            //getResponseResponse.AudioResponse.TextData = getResponseResponse.AudioResponse.TextData.Substring(0, getResponseResponse.AudioResponse.TextData.LastIndexOf(' ')).TrimEnd();
                            //getResponseResponse.AudioResponse.TextData = getResponseResponse.AudioResponse.TextData.Substring(0, getResponseResponse.AudioResponse.TextData.LastIndexOf(' ')).TrimEnd();
        ...
        /// </remarks>
        private AudioClip TrimAudioClip(AudioClip sourceClip, float startTime, float endTime)
        {
            // Yeni clip uzunluğunu belirle
            float newClipLength = endTime - startTime;

            // Yeni clip oluştur
            AudioClip newClip = AudioClip.Create("TrimmedClip", (int)(newClipLength * sourceClip.frequency), sourceClip.channels, sourceClip.frequency, false);

            // Verileri kopyala
            float[] data = new float[(int)(newClipLength * sourceClip.frequency * sourceClip.channels)];
            sourceClip.GetData(data, (int)(startTime * sourceClip.frequency * sourceClip.channels));
            ...
            while (!_playingStopLoop)
                ...
                if (_responseAudios.Count > 0)
                {
                    ...
                    if (!currentResponseAudio.isFinal)
                    {
                        ...
                        /*
                        if(_responseAudios.Count > 1){
                            if(_responseAudios[1].isFinal){
                            _audioSource.SetScheduledEndTime((_audioSource.clip.length - 1.5) + AudioSettings.dspTime + 0.1f);
                            }
                        }else{
                            _audioSource.SetScheduledEndTime((_audioSource.clip.length) + AudioSettings.dspTime + 0.1f);
                        } */
                        
                        if(playTheAud == true){
                            _audioSource.SetScheduledEndTime((_audioSource.clip.length - 1.7) + AudioSettings.dspTime);
                            playTheAud = false;
                        }else{
                            _audioSource.SetScheduledEndTime((_audioSource.clip.length) + AudioSettings.dspTime);
                        }
                        if(isScored == true){
                            playTheAud = true;
                        }
